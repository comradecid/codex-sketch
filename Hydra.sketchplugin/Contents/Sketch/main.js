...
exports['default'] = function (context) {
	...
	function processLayer(layer) {
		...
		var layerName = layer.sketchObject.className();

		// If this is a symbol...
		// TODO: handle diffs between instances and masters
		// TODO: Recurse this!
		if (layerName == LAYER_TYPE_SYMBOL_INSTANCE || layerName == LAYER_TYPE_SYMBOL_MASTER) {

			// Get JSON for selected symbol
			// TODO: Non-JSON formatting of symbols collection info can lead to parse errors
			var jsonString = (0, _common.getLayerJSON)(layer);
	...
	if (selectedLayers.length > 0) {
		...
		if (numSymbols > 0) {
			...
			var clickEvent = (0, _common.getConfirmation)(sketch, foundSymbols);
			...
			if (clickEvent == NSAlertFirstButtonReturn) {
				...
				if (DEBUG) {
					...
					(0, _common.dumpToOutputFile)(JSON.stringify(foundSymbols, null, "\t"));
				} else {
			...
			sketch.message(_common.MSG_SELECT_SYMBOL);
		}
...
var CONSOLE_ERR_PRFX = '[HYDRA](ERR) ';
...
var LBL_TITLE_CONFIRM_UPDATE = 'Confirm update to style guide';
var MSG_CONFIRM_CONFIRM_UPDATE = 'Would you like to update the style guide with the following symbols?';
